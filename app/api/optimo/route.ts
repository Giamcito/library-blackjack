...
import { optimo } from "@/lib/algorithms"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (USE_LOCAL) {
      const data = optimo(referencias, marcos)
      return NextResponse.json(data)
    }
    const remote = await callRemote("optimo", referencias, marcos)
    return NextResponse.json(remote)
  } catch (error) {
    console.error("Error Óptimo:", error)
